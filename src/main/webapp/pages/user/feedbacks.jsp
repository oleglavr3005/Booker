...
<link
	href="${pageContext.servletContext.contextPath}/resources/css/rating.css"
	rel="stylesheet">
<link rel="stylesheet"
	href="${pageContext.servletContext.contextPath}/resources/css/jPage/style.css">
...
<c:if test="${fn:length(feedbacks) == 0}">
	...
	<img
		src="${pageContext.servletContext.contextPath}/resources/images/comment.png"
		alt="comments" class="img-responsive center-block"
					...
					<div class="media col">
						<div class="media-body">
							<div class="col s6" style="margin-top: 15px; padding-left: 0;">
								<input id="title_feedback_${feedback.id}" class="validate"
								
								
									<c:if test="${user.status == 'BANNED'}">
								disabled
						</c:if>
									type="text" value="<c:out value="${feedback.title}"/>">
								<label id="title_feedback_${feedback.id}_label" data-error=""
									for="title_feedback_${feedback.id}" class=""> <span
									id="title_feedback_span_${feedback.id}"></span>
								</label>
							</div>
							<div class="col s6" style="margin-top: 15px;">
								<div class="vote-block" rel="v:rating"
									...
									<ol class="rating show-current">
										...
										<li class="current"><span
											style="width:${feedback.rating * 20}%"></span></li>
									</ol>
									<span class="manager_hotel_star"
										style="font-size: 14; position: relative; left: 10px; top: 8px;"></span>
									<span id="rate_span"
										style="font-size: 14; position: relative; left: 10px; top: 8px;">${feedback.rating}</span>
									<span
										style="font-size: 14; position: relative; left: 10px; top: 8px;">
										/ 5</span>
								</div>
							</div>
						</div>
						<textarea id="feedback_${feedback.id}"
							...
							style="padding-top: 0;"><c:out
								value="${feedback.comment}" /></textarea>

						<c:if test="${user.status != 'BANNED'}">
							<div id="button_panel_${feedback.id}" class="row"
								style="text-align: center;">
								<a class="waves-effect waves-light btn"
									onclick="deleteFeedback(${feedback.id})"
									style="margin-left: 10px; background: #F55151; color: #F7F7F7; font-family: 'Times NewRoman', Times, serif;">
									<span class="feedback_error_${feedback.id}"></span> <span
									class="my_feedback_delete"></span>
								</a> <a class="waves-effect waves-light btn"
									onclick="updateFeedback(${feedback.id})"
									style="background: #26A69A; text-align: center; color: #F7F7F7;">
...
<script type="text/javascript"
	src="${pageContext.servletContext.contextPath}/resources/js/hotel/rating.js"></script>