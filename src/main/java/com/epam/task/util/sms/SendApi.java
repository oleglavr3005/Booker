...
public class SendApi {
	...
	private String SMSC_PASSWORD = "<password>"; // пароль или MD5-хеш пароля в
													// нижнем регистре
	private boolean SMSC_HTTPS = false; // использовать HTTPS протокол
	private String SMSC_CHARSET = "utf-8"; // кодировка сообщения: koi8-r,
											// windows-1251 или utf-8 (по
											// умолчанию)
	...
	public SendApi(String login, String password) {
		SMSC_LOGIN = login;
		SMSC_PASSWORD = password;
	}

	public SendApi(String login, String password, String charset) {
		SMSC_LOGIN = login;
		SMSC_PASSWORD = password;
		SMSC_CHARSET = charset;
	}

	public SendApi(String login, String password, String charset, boolean debug) {
	        SMSC_LOGIN    = login;
	        SMSC_PASSWORD = password;
	        SMSC_CHARSET  = charset;
	        SMSC_DEBUG    = debug;
	...
	public String[] sendSms(String phones, String message, int translit, String time, String id, int format,
		...
		if (SMSC_DEBUG) {
			if (Integer.parseInt(m[1]) > 0) {
				System.out.println("Сообщение отправлено успешно. ID: " + m[0] + ", всего SMS: " + m[1]
	...
	public String[] getSmsCost(String phones, String message, int translit, int format, String sender, String query) {
		...
		try {
			m = SmscSendCmd("send",
					"cost=1&phones=" + URLEncoder.encode(phones, SMSC_CHARSET) + "&mes="
							+ URLEncoder.encode(message, SMSC_CHARSET) + "&translit=" + translit
							+ (format > 0 ? "&" + formats[format] : "")
							+ (sender == "" ? "" : "&sender=" + URLEncoder.encode(sender, SMSC_CHARSET))
							+ (query == "" ? "" : "&" + query));
		} catch (UnsupportedEncodingException e) {
